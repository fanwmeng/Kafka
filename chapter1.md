...
这里说将如上类目整合在一起，就是上边各个子主题。即将消息、存储和流处理整合在一起看起来有点怪怪的，但是对Kafka这种流处理平台来说是至关重要必不可少的。

像HSFS这种分布式文件系统允许存储静态文件来进行批处理。像这样的系统允许地有效地存储和处理过去的历史数据。

传统的企业消息系统允许处理订阅后收到的消息。以这种方式构建的应用也是能处理将要收到的消息。

Kafka组合了这两种能力，而这种对两种能力的组合对使用Kafka作为流式应用平台和数据流管道来说是非常重要的。

通过对存储和低延迟订阅的组合，流式应用程序可以对过去和未来的数据以相同的方式处理。这是一个单独的应用程序，能够处理过去的存储起来的数据，而不是当最后一条消息到达时就结束了。它能够继续处理接下来到达的消息。这就是平常所说的流处理的概念即包含批处理和消息驱动程序。

同样的对数据流管道来说，对实时事件的订阅组合使得处理低延迟的管道可以使用Kafka；但是可靠的存储数据的能力使得Kafka可以用于必须保证传输的关键数据，定期加载数据的离线系统，或者可能长时间停机维护。流式处理可以在数据到达时完成转换。
